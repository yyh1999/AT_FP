...
class BaselineTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        optimizer = self.build_optimizer(cfg, model,True)
...
class ATeacherTrainer(DefaultTrainer):
    def __init__(self, cfg):
        ...
        #############2023.03.22#############################
        optimizer_teacher = self.build_optimizer(cfg, self.model_teacher,False)
    ...
    def run_step_full_semisup(self):
        ...
        if self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP * 0.4:

            # input both strong and weak supervised data into model
            label_data_q.extend(label_data_k)
            record_dict, _, _, _ = self.model(
                label_data_q, branch="warm_up_supervised")
        ...
        elif self.cfg.SEMISUPNET.BURN_UP_STEP * 0.4 <= self.iter < self.cfg.SEMISUPNET.BURN_UP_STEP:
            ...
            (
                ...
                _,
                _,
            ) = self.model_teacher(unlabel_data_k, branch="unsup_data_nicl")
            
            #record_dict.update(record_nicl)
            #############2023.03.22#############################
            self.optimizer_teacher.zero_grad()
            record_nicl_loss = sum(record_nicl.values())
            record_nicl_loss.backward()
    ...
    def _write_metrics(self, metrics_dict: dict):
        ...
        if comm.is_main_process():
            ...
            #修改 以便loss_nice出现nan没问题
            ####################################################################################
            metrics_dict = {}
            for d in all_metrics_dict: #遍历该列表，每个list item是字典
                for k in d:  #d是字典 k是key
                    if k not in metrics_dict:
                        metrics_dict[k] = d[k] 
                    else:
                        metrics_dict[k] += d[k] 

            metrics_dict = {k:metrics_dict[k]/len(all_metrics_dict) for k in metrics_dict}
    ...
    def _update_teacher_model(self, keep_rate=0.9996):
        ...
        for key, value in self.model_teacher.state_dict().items():
            if key in student_model_dict.keys():
                if key not in ['NF','NS','queue_ptr']:
                    new_teacher_dict[key] = (
                        student_model_dict[key] *
                        (1 - keep_rate) + value * keep_rate
                    )
                else: #key = NF/NS/queue_ptr
